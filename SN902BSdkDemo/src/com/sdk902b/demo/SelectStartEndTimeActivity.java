...
//		wvAPM = (WheelView) findViewById(R.id.apm);
	...
	protected void initUI() {
		...
		sHour = intent.getByteExtra(EXTRA_START_HOUR, (byte)0);
		sMinute = intent.getByteExtra(EXTRA_START_MINUTE, (byte)0);
		eHour = intent.getByteExtra(EXTRA_END_HOUR, (byte)0);
		eMinute = intent.getByteExtra(EXTRA_END_MINUTE, (byte)0);
...
//		}
	...
	private void setHourTime(int type, int index) {
		...
		if (type == START_TIME) {
			...
			if (true) {
				...
				eHour = TimeUtil.getHour24(index, eMinute, newEndItem);
				lastPosition = index;
			}
			setEndTimeText();
		}
	...
	private void setStartTimeText() {
		if (true) {
			...
			tvStartTime.setText(StringUtil.DF_2.format(TimeUtil.getHour12(sHour))
					+ ":"
					+ StringUtil.DF_2.format(sMinute)
	...
	private void setEndTimeText() {
		if (true) {
			...
			tvEndTime
					.setText(StringUtil.DF_2.format(TimeUtil.getHour12(eHour))
							+ ":"
							+ StringUtil.DF_2.format(eMinute)
	...
	public void onClick(View v) {
		super.onClick(v);
		if(v == tvRight) {
			SdkLog.log(TAG+" onClick sH:" + sHour+",sM:" + sMinute+",eH:" + eHour+",eM:" + eMinute);
			...
			if (v.getTag() == null) {
				...
				v.setBackgroundResource(R.drawable.time_sel);
